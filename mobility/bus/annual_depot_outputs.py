...
def build_run_summary_markdown(
    *,
    ...
    total_energy = _sum(vehicle_day_soc_summary, "total_energy_kwh")
    total_deadhead = _sum(vehicle_day_soc_summary, "total_deadhead_km")
    feasible_share = (
        float(vehicle_day_soc_summary["depot_only_feasible"].astype(bool).mean())
        if not vehicle_day_soc_summary.empty and "depot_only_feasible" in vehicle_day_soc_summary.columns
        else np.nan
    )
    lines = [
        ...
        f"- n_unassigned_block_instances_under_ev_stock_scale: {_assignment_unassigned_total(vehicle_day_assignments)}",
        f"- mean_daily_assignment_coverage_share: {_format_float(_assignment_mean_coverage(vehicle_day_assignments))}",
        f"- n_bus_trip_records: {_len_or_zero(bus_trip_records)}",
        f"- n_bus_charging_events: {_len_or_zero(bus_charging_events)}",
        f"- n_bus_ev_state_records: {_len_or_zero(bus_ev_state_records)}",
    ...
    lines.extend(_top_blocks_by_shortfall(vehicle_day_soc_summary))
...
def _daily_assignment_diagnostics(assignments: pd.DataFrame) -> pd.DataFrame:
    required = {
        "service_date",
    ...
    if assignments.empty or not required.issubset(assignments.columns):
        return pd.DataFrame()
    return assignments.loc[:, sorted(required)].drop_duplicates("service_date")


def _assignment_unassigned_total(assignments: pd.DataFrame) -> int:
    daily = _daily_assignment_diagnostics(assignments)
    if daily.empty:
        return 0
    return int(pd.to_numeric(daily["n_unassigned_block_instances_for_service_date"], errors="coerce").fillna(0).sum())


def _assignment_mean_coverage(assignments: pd.DataFrame) -> float:
    daily = _daily_assignment_diagnostics(assignments)
    if daily.empty:
        return np.nan

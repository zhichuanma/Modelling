...
5. 调用方（[sim_adapter.py:43](../../../mobility/coach/sim_adapter.py#L43)、[notebooks/_build_02_coach_narrative.py](../../../notebooks/_build_02_coach_narrative.py)）一并改为按位置读 `Energy_kWh / consumption_kwh_per_km`
...
下游 [sim_adapter.py](../../../mobility/coach/sim_adapter.py) 与 [notebooks/_build_02_coach_narrative.py](../../../notebooks/_build_02_coach_narrative.py) Stage E 相应改字段名。
...
[_build_02_coach_narrative.py:295](../../../notebooks/_build_02_coach_narrative.py#L295) 用 `usable = battery_kwh * 0.95` 内联做 5% safety margin。改用 `journey_feasibility(...)` 调用结果，避免默认值漂移导致 frontier 与 `result["feasibility"]` 不一致。
